...
def slurp(url, data=None, headers={}, timeout=60):
    ...
    return urllib.request.urlopen(urllib.request.Request(url, data, headers), None, timeout).read()
...
def nextbus_stop_vehicle(agency, route, stop, vehicleid):
  ...
  for vid, (routeTag, dirTag, lat, lon, age, heading, predictable) in list(vehicles.items()):
    seen(lat, lon)
    if vid == vehicleid:
...
def nextbus_stop_relative(agency, route, stop, relative):
  ...
  if len(options) == 1:
    stop_identifier, stop_name = list(options.keys())[0]
  ...
  escaped_content = []
  for (stop_identifier, stop_name), directions in list(options.items()):
